...
- **图片遮罩**：用白色或黑色矩形盖住不需要识别的内容，支持多个遮罩、移动、缩放、删除和撤销。
...
3. 需要排除选区内部的内容时，点击 **白色遮罩** 或 **黑色遮罩**，拖动绘制矩形；可拖动遮罩移动，拖动边缘与四角缩放。点击 **调整选区** 可继续调整截图范围。
4. 按 `Enter` 或点击 **识别** 提交；`Esc` 或右键取消。
5. 识别后核对预览和原图，点击公式即可复制 LaTeX，也可在源码区域修改内容。

方向键移动选区，`Shift + 方向键` 加大移动步长。多显示器时截取鼠标所在屏幕；快捷键被占用时会提示。

普通导入或粘贴的图片先保存在本机，点击 **开始识别** 后上传。图片识别默认在后台准备官方 Word，点击 **导出 Word** 保存并打开。

导入或历史图片也可切换到 **原图 → 添加遮罩**。编辑后点击 **保存遮罩图片**，会另存一条新记录并保留原图，再点击 **开始识别** 提交。遮罩支持 `Delete` 删除所选、`Ctrl + Z` 撤销；选中遮罩后，方向键微调位置，`Shift + 方向键` 加大步长。每张图片最多添加 100 个遮罩。

遮罩会实际写入保存和提交的图片像素；关闭编辑窗口不保存。编辑历史图片不会修改原记录及其已有识别结果。

底部的 **Mathpix 整体置信度** 使用官方返回的顶层 `confidence`，表示结果完全正确的估计概率，不使用 `confidence_rate` 代替。旧记录从已保存的原始响应中读取，无需重新识别；未返回该字段时显示“未提供整体置信度”。编辑正文后标注为原始结果的置信度，该数值不是人工校验后的准确率。字段定义见 [Mathpix 图片接口文档](https://docs.mathpix.com/reference/post-v3-text)。
