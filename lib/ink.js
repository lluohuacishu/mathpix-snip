...
export class InkRecognition {
  ...
  async run(id,file,hash,strokes,texts,layoutImage) {
    ...
    if(previous) {
      ...
      if(previous.status==='completed') return {...previous.result,confidence:overallConfidence(previous.raw)};
    ...
    try {
      ...
      const result={id,latex,mmd,confidence:overallConfidence(raw),requestId:raw.request_id,mathGroups:strokes.x.length?1:0,mode};
  ...
  async save(id) {
    ...
    if(existing) return {...existing,confidence:overallConfidence(existing.raw)};
    return this.store.put({id:itemId,kind:'text',title:'手写公式 '+new Date(entry.createdAt).toLocaleString('zh-CN'),createdAt:entry.createdAt,mmd:entry.result.mmd,confidence:overallConfidence(entry.raw),strokes:entry.strokes,inkTexts:entry.texts||[],inkMode:entry.result.mode,raw:entry.raw,status:'completed',demo:false,wordRequested:false});
  }
}
