...
export class PdfJobs {
  constructor({store,client,update,word,directory,openWord = openWordFile,openFolder = openFolderPath,pollMs = 3000,maxPolls = 28800}) {
  ...
  async submit(item,{mode='files',pages=''}) {
    ...
    item.pdfTask = { mode, pages:selection.ranges, selectedPages:selection.count, estimatedCost:selection.count ? selection.count*PDF_RATES[mode] : null, status:'submitting', artifacts:{}, createdAt:new Date().toISOString() };
  ...
  checkFolder(task) {
    if (path.dirname(path.resolve(task.folder)) !== this.directory) throw new Error('PDF 输出文件夹不在指定目录中。');
  }
  async publish(id,key,bytes) {
    ...
    await this.update(id,async item => {
      ...
      if (!task.folder) Object.assign(task,await reserveBundle(this.directory,item.title));
