...
export async function saveAndOpenWord({ buffer, title, directory = defaultWordDirectory, openFile = openWordFile, autoOpen = true }) {
  ...
  if (!autoOpen) return { path:file, opened:false, autoOpen:false };
  try {
    await openFile(file);
