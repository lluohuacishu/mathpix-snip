  ...
  async list() {
    ...
    const items = await Promise.all(entries.filter(n => n.endsWith('.json')).map(async n => {
      try {
        ...
        return { id: i.id, title: i.title, createdAt: i.createdAt, kind: i.kind, status: i.status, pdfStatus:i.pdfTask?.status, demo: i.demo, favorite: !!i.favorite, excerpt: (i.mmd || '').slice(0, 110) };
      } catch { return null; }
    }));
